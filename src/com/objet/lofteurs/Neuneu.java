...
public class Neuneu {
	...
	public void action() {
		...
		//Si la cellule contient de la nourriture et si le neuneu n'est pas rassasiŽ
		else if(this.getCelluleCourante().getNourriture() != null && this.getEnergie()<Saison1.maxEnergie){
			int k = (int)(2*(Math.random())+1);
			switch (k) {
				case (1) : this.marcher();
				System.out.println(this + " marche alors qu'il a la possibilitŽ de manger");
				break;
				case (2) : this.manger();
		...
		System.out.println("QuantitŽ energie du neuneu aprs action : " +this.getEnergie());
	}
	...
	public void manger(){
		if (this.celluleCourante.getNourriture() != null){
			...
			System.out.println("QuantitŽ nourriture dans la cellule aprs repas : " + cellNourriture.getQuantite());
			}
		}
	...
	public void marcher() {
		...
		//Fait diminuer l'Žnergie du Neuneu
		this.setEnergie(this.getEnergie()-2); // Ca creuse de marcher !
	}

	public void seReproduire(int x,int y) {
		Neuneu enfant = new Neuneu(loft,x,y);
		loft.add(enfant);
